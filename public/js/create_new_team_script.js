...
function getEditURL(index) {
  window.location.href = `/edit-player?team=${$(
    "#team-name"
  ).val()}&player=${index}`;
}
...
$(document).ready(async function (event) {
  ...
  if (location.href.search("new-team") == -1) {
    if (team == null) window.location.replace("/myteams");
    else {
      ...
      if (flag == false) window.location.replace("/myteams");
      else {
        $(document).prop("title", "SCORE-SIZZLE - Edit Team");
    ...
    $(document).prop("title", "SCORE-SIZZLE - Create New Team");
