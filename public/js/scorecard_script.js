...
let displayPrediction = false;

function savePrediction(probability) {
  if (probability !== undefined && probability !== null) {
    localStorage.setItem("matchPrediction", probability.toString());
  }
}
...
function updatePredictionUI(probability) {
  ...
  const winProbability = parseFloat(probability);
  const lossProbability = 1 - winProbability;

  const formattedWinProb = (winProbability * 100).toFixed(1);
  const formattedLossProb = (lossProbability * 100).toFixed(1);

  // Update team probabilities
  const teamAProb = document.getElementById("team-a-probability");
  const teamBProb = document.getElementById("team-b-probability");
  
  if (teamAProb) {
      teamAProb.textContent = `${formattedWinProb}%`;
  }
  if (teamBProb) {
      teamBProb.textContent = `${formattedLossProb}%`;
  }

  // Update the probability bar
  const predictionBar = document.getElementById("prediction-bar");
  if (predictionBar) {
      predictionBar.style.width = `${formattedWinProb}%`;
  }
}

// Function to set team names
function setTeamNames(teamA, teamB) {
  const teamElements = document.getElementsByClassName('team-name');
...
async function callPredictionAPI(data) {
  try {
    ...
    const probability = result.win_probability;

    // Save prediction before updating UI
    savePrediction(probability);
    updatePredictionUI(probability);
...
function calculatePredictionData() {
  ...
  let currentRunRate =
    overs == 0
      ? 0
      : runs / (parseInt(overs.toFixed(1).split(".")[0]) + over_counter / 6);

  // Calculate remaining overs properly accounting for partial overs
  let remainingOvers =
    inning == 2
      ? currentOvers.toString().split(".")[1] == "0"
        ? totalOvers - currentOvers
        : totalOvers - currentOvers - 0.4
      : 0;

  // Calculate required run rate for second innings
  let requiredRunRate = 0;
  if (inning == 2 && remainingOvers > 0) {
    requiredRunRate = (
      (target - runs) /
      (parseInt(remainingOvers.toString().split(".")[0]) +
        parseInt(remainingOvers.toString().split(".")[1] || 0) / 6)
    ).toFixed(2);
  }
  ...
  return {
    ...
    required_run_rate: parseFloat(requiredRunRate),
  };
}
...
async function reloadPageWithPrediction() {
...
for (let i = 0; i < 10; i++) {
  if (i == 1)
    $(".runs-area").append(
      `<button class='run-btn' style='padding: 4px 11px;' id='${i}'>${i}</button>`
    );
  else
    $(".runs-area").append(`<button class='run-btn' id='${i}'>${i}</button>`);
}
$(".runs-area").append(
  `<div class="custom-runs"><input type="text" title="Enter custom runs here" name="custom-runs" class="custom-input" required><span class="material-icons custom-tick">check_circle</span></div>`
);
...
for (let i = 0; i < wicket_types.length; i++) {
  $(".wickets-area").append(
    `<button class="run-btn wicket-btn" name=${i}>${wicket_types[i]}</button>`
  );
}
...
extras.forEach((extra) => {
  $(".extras-area").append(
    `<button class="run-btn extras-btn" name="${extra[1]}" title="${extra[1]}">${extra[0]}</button>`
  );
});
...
$(".inningContent").css(
  "width",
  `calc(${$(".scorecard-section").css("width")} - 50px)`
);
...
function switchTab(index) {
  $(".tab-container").css(
    "margin-left",
    `calc(-${$(".scorecard-section").css("width")} * ${index})`
  );
  $(".inningButtons button").removeClass("active");
  $("#hline").css(
    "margin-left",
    `calc(${$(".inningButtons button").css("width")} * ${index})`
  );
...
$(document).ready(async (event) => {
  if (document.cookie.search("db") == -1 && id != null) {
    db = CryptoJS.AES.decrypt(
      new URLSearchParams(window.location.search).get("id").toString(),
      "scorego"
    ...
    await fetch(`/live-scorecard/${id}/check-match`, {
      ...
      .then((res) => {
        ...
        } else if (res.present) window.location.replace(`/match-summary/${id}`);
  ...
  await fetch(`/live-scorecard/${id}/fetch-match-info`, {
    ...
    .then(async (res) => {
      ...
      res.team.forEach((ele) => {
        if (ele.name == bowling_team) keeper = ele.players[ele.keeper].name;
      });

      $(document).prop(
        "title",
        `${title} - ${res.team[0].abbr} vs ${res.team[1].abbr} Live Scorecard - SCORE-SIZZLE`
      );
      ...
      if (res.match_info.umpires[0] != res.match_info.umpires[1])
        $(".umpires .info").html(
          `${res.match_info.umpires[0]}, ${res.match_info.umpires[1]}`
        );
      if (inning == 1) {
        $(".required-rr, .remaining").hide();
        $("#inning, .innings_div .info").html("1st Innings");
        // <span id="inning-no" hidden></span> set it as 1
        $("#inning-no").html(1);
        //<section class="prediction-section" id = "prediction"> hide this
        ...
        $(".inningButtons button").eq(1).html(batting_team);
         // <div class="team-name-1" id="team1"></div> set it as batting team
         $(".team-name-1").html(batting_team);
         // <div class="team-name-2" id="team2"></div> set it as bowling team
         $(".team-name-2").html(bowling_team);

        target = res.target;
        let rem_overs =
          res.inning_data.overs.toFixed(1).split(".")[1] == "0"
            ? res.match_info.overs - res.inning_data.overs
            : res.match_info.overs - res.inning_data.overs - 0.4;
        let rem_runs = res.target - res.inning_data.runs;
        let req_rr = (
          rem_runs /
          (parseInt(rem_overs.toFixed(1).split(".")[0]) +
            parseInt(rem_overs.toFixed(1).split(".")[1]) / 6)
        ).toFixed(2);
        $(".required-rr .info").html(req_rr);
        $(".remaining span").html(
          res.inning_data.bat == res.team[0].name
            ? res.team[0].abbr
            : res.team[1].abbr
        );
        $(".remaining .info:first").html(rem_runs);
        $(".remaining .info:last").html(
          parseInt(rem_overs.toFixed(1).split(".")[0]) * 6 +
            parseInt(rem_overs.toFixed(1).split(".")[1])
        );
      ...
      await fetch_scorecard(1).then(async function () {
        if (
          parseInt($(".scorecard-section").css("height")) >
          parseInt($(".left-side").css("height"))
        ) {
          $(".overs-timeline").css(
            "height",
            $(".scorecard-section").css("height")
          );
        ...
        for (let i = 0; i < 2; i++) {
          ...
          let j;
          if (
            $(`#t${i + 1}-block .team-name`).html() ==
            $(".inningButtons button").eq(i).html()
          )
            j = i;
          else j = 1 - i;
          $(`.t${j + 1} tr th`).css("background-color", res.team[i].color);
          $(`.t${j + 1} tr:last-child td`).css(
            "border-bottom",
            `3px solid ${res.team[i].color}`
          );
          $(`.t${j + 1} tr:nth-child(odd)`).css(
            "background-color",
            res.team[i].color + "30"
          );
          ...
          if (batting_team == res.team[i].name) {
            ...
            $(`#t${1 - i + 1}-block div img`).attr(
              "src",
              "../img/ball-icon.ico"
            );
            $(`#t${1 - i + 1}-block div`).css(
              "background-color",
              res.team[1 - i].color
            );
            $(".main-score-pane").css({
              border: `3px solid ${res.team[i].color}`,
              color: res.team[i].color,
            });
            $(".main-score-pane div:nth-child(2)").css(
              "background-color",
              res.team[i].color
            );
        ...
        for (let j = 0; j < res.inning_data.batting.length; j++) {
          if (res.inning_data.batting[j].status == "not out") {
            $(`#batsman${k} .name`).html(
              res.inning_data.batting[j].name.split(" ")[0].substring(0, 1) +
                " " +
                res.inning_data.batting[j].name.split(" ")[1].toUpperCase()
            );
        ...
        if (k != 3)
          await fetch_players_popup(batting_team, "NEXT BATSMAN").then(() =>
            $(".overlay").css("display", "flex")
          );

        for (let j = 0; j < res.inning_data.bowling.length; j++) {
          if (
            res.inning_data.bowling[j].name ==
            res.inning_data.timeline[res.inning_data.timeline.length - 1].name
          ) {
            bowler = res.inning_data.bowling[j].name;
            $("#bowler .name").html(
              res.inning_data.bowling[j].name.split(" ")[0].substring(0, 1) +
                " " +
                res.inning_data.bowling[j].name.split(" ")[1].toUpperCase()
            );
            $("#bowler .runs").html(res.inning_data.bowling[j].runs);
            $("#bowler .wickets").html(res.inning_data.bowling[j].wickets);
            $("#bowler .overs").html(
              parseFloat(res.inning_data.bowling[j].overs).toFixed(1)
            );
          }
        }
        ...
        $("#run-rate").html(
          (overs == 0
            ? 0
            : runs /
              (parseInt(overs.toFixed(1).split(".")[0]) + over_counter / 6)
          ).toFixed(2)
        );
        ...
        setTimeout(() => {
          if (document.cookie.search("db") != -1) {
            Swal.fire({
              ...
              confirmButtonText:
                'Will resume soon <i class="fa fa-thumbs-up"></i>',
              cancelButtonText:
                'Will take some more time <i class="fa fa-thumbs-down"></i>',
          ...
          } else $("#live").html("GO LIVE");
        }, 300000);

        if ($("body").width() > 1100) $("#preloader").css("display", "none");
      });
    });
});
    ...
    .then((res) => {
      for (let j = 0, k = 1; j < res.batting.length; j++, k++) {
        ...
        $(`#tab${inn} .shadow:first-child .score-table`).append(`
					<tr>
						...
						<td>${(res.batting[j].balls == 0
              ? 0
              : (res.batting[j].runs / res.batting[j].balls) * 100
            ).toFixed(2)}</td>
					</tr>
				`);
      }
      ...
      $(`#tab${inn} .shadow:first-child .score-table`).append(`
            ...
            res.extras.penalty
					...
					<td colspan="4">(${res.extras.wide} wd, ${res.extras.no_ball} nb, ${
        res.extras.leg_byes
      } lb, ${res.extras.byes} b, ${res.extras.penalty} p)</td>
				</tr>
				<tr class="stats">
					<td>Total Runs</td>
					<td>${res.runs}</td>
        ...
        $(`#tab${inn} .shadow:first-child .score-table`).append(`
					<tr class="yet-to-bat">
						<td colspan="6">Yet to Bat<br>
							<span class="batsman-status">${yet_to_bat.substring(
                0,
                yet_to_bat.length - 3
              )}</span>
						</td>
      ...
      for (let j = 0; j < res.bowling.length; j++) {
        $(`#tab${inn} .shadow:last-child .score-table`).append(`
					<tr>
						...
						<td>${(res.bowling[j].overs == 0
              ? 0
              : res.bowling[j].runs /
                (parseInt(res.bowling[j].overs.toFixed(1).split(".")[0]) +
                  parseInt(res.bowling[j].overs.toFixed(1).split(".")[1]) / 6)
            ).toFixed(2)}</td>
					</tr>
				`);
      }
  ...
  await fetch(`/live-scorecard/${id}/fetch-players-popup`, {
    ...
    .then((res) => {
      ...
      for (let j = 0; j < res.players.length; j++) {
        $(".players").append(
          `<li><p>${res.players[j].name}</p><span class="material-symbols-rounded">done</span></li>`
        );
        if (res.captain == j) $(".players li p").eq(j).append(" (C)");
        if (res.vice_captain == j) $(".players li p").eq(j).append(" (VC)");
        if (res.keeper == j) $(".players li p").eq(j).append(" (WK)");
      }
      ...
      $(".players-popup li").click(function (event) {
        if ($(".players-popup li.active").length == 1) {
          ...
          if (status == "OVER COMPLETE") {
            for (let j = 0; j < res.players.length; j++) {
              if (
                res.players[j].bowl == "none" ||
                res.players[j].name == bowler
              ) {
                $(".players-popup li").eq(j).addClass("disabled");
              }
            }
            ...
            for (let j = 0; j < res.players.length; j++) {
              for (let k = 0; k < res.batting.length; k++) {
                if (
                  res.players[j].name == res.batting[k].name &&
                  res.batting[k].status != "retired hurt"
                ) {
                  $(".players-popup li").eq(j).addClass("disabled");
                  break;
                }
  ...
  if (inning == 1) {
    await fetch(`/live-scorecard/${id}/check-end-of-innings`, {
      ...
      .then(async function (res) {
        ...
        if (
          res.match_overs == res.inning_overs ||
          res.wickets + res.retired_hurt == 10
        ) {
          Swal.fire({
            icon: "info",
            title: "End of Innings!",
            html: `<b>Total Score:</b> ${res.runs}<br><b>Target:</b> ${
              res.runs + 1
            }`,
            confirmButtonText: "OK",
            confirmButtonColor: "#4153f1",
      ...
      case "OVER COMPLETE":
        await fetch(`/live-scorecard/${id}/change-bowler`, {
          ...
          body: JSON.stringify({
            ...
            prev_bowler: bowler,
            new_bowler:
              $(".players-popup li.active p").html().indexOf("(") == -1
                ? $(".players-popup li.active p").html()
                : $(".players-popup li.active p")
                    .html()
                    .slice(
                      0,
                      $(".players-popup li.active p").html().indexOf("(") - 1
  ...
  .click(async function () {
    ...
    if (custom_runs == -1) {
      ...
      $(".runs-area")
        .eq(0)
        .find(".custom-input")
        .css("border", "2px solid var(--primary-color)");
      custom_runs = -1;
    }
        ...
        .then(async (res) => {
          if (res.updated) {
            Swal.fire({
              ...
              await check_end_of_innings();
              if (overs % 1 == 0)
                await fetch_players_popup(bowling_team, "OVER COMPLETE").then(
                  () => $(".overlay").css("display", "flex")
                );
              else window.location.reload();
              await updatePrediction();
...
$(".extras-area .extras-btn").click(function (event) {
  ...
  Array.from($(".extras-area .extras-btn")).forEach(function (ele) {
    if ($(ele).html() != active_ele.html()) $(ele).removeClass("active");
  });
  if ($(".extras-area .extras-btn").hasClass("active"))
    $(".extras-dropdown").addClass("active");
  else $(".extras-dropdown").removeClass("active");
});
...
$(".extras-dropdown")
  ...
  .click(async function (event) {
    ...
    if (custom_extras == -1) {
      ...
      $(".extras-dropdown .custom-input").css(
        "border",
    ...
    if ($(".extras-area .extras-btn").hasClass("active") && runs_scored != -1) {
      if (
        new Array("Wide", "No Ball").includes(
          $(".extras-area .extras-btn.active").attr("name")
        )
      ) {
        await fetch(`/live-scorecard/${id}/add-extras1`, {
          ...
          .then(async (res) => {
            if (res.updated) {
              Swal.fire({
                icon: "success",
                title: `${$(".extras-area .extras-btn.active").attr(
                  "name"
                )} - ${runs_scored} Runs`,
                showConfirmButton: false,
                timer: 1500,
      ...
      if (wicket_index == 0) status = `(b) ${bowler}`;
        ...
        await fetch(`/live-scorecard/${id}/fetch-players-popup`, {
          ...
          .then(async function (res) {
            ...
            if (fielder != undefined) status = `(c) ${fielder} (b) ${bowler}`;
            else return;
          });
      }
        ...
        .then(async (res) => {
          if (res.updated) {
            Swal.fire({
              ...
              title: "Wicket!",
              text: bowling_comp[
                Math.floor(Math.random() * bowling_comp.length)
              ],
              showConfirmButton: false,
              timer: 1500,
            }).then(async function () {
              ...
              } else await fetch_players_popup(batting_team, "NEXT BATSMAN").then(() => $(".overlay").css("display", "flex"));
            });
          }
        });
      break;
                ...
                .then(async (res) => {
                  if (res.updated) {
                    Swal.fire({
                      ...
                      title: "Wicket!",
                      text: run_out_comp[
                        Math.floor(Math.random() * run_out_comp.length)
                      ],
                      showConfirmButton: false,
                      timer: 1500,
                    }).then(async function () {
                      ...
                      await check_end_of_innings();
                      if (overs % 1 == 0)
                        await fetch_players_popup(
                          bowling_team,
                          "OVER COMPLETE"
                        ).then(() => $(".overlay").css("display", "flex"));
                      else
                        await fetch_players_popup(
                          batting_team,
                          "NEXT BATSMAN"
