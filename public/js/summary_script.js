...
if (document.cookie.search("db") == -1) window.location.replace("/get-started");
...
function switch_tab(index) {
  ...
  $("#hline").css(
    "margin-left",
    `calc(${$(".summaryButtons li").css("width")} * ${index})`
  );
...
$(document).ready(async function () {
  await fetch(`/match-summary/${id}/check-match`, {
    ...
    .then((res) => {
      if (res.exists) {
        if (!res.end) window.location.replace(`/start-match/${id}`);
  ...
  await fetch(`/match-summary/${id}/fetch-match-info`, {
    ...
    .then(async (res) => {
      ...
      $(document).prop("title", `${title} Summary - SCORE-SIZZLE`);
      ...
      for (let i = 0; i < 2; i++) {
        ...
        $(`#total${i + 1}`).html(
          res.innings[i].runs + "/" + res.innings[i].wickets
        );
        ...
        for (let j = 0; j < 4; j++) {
          $(`#t${i + 1} table:first-child`).append(`
                    ...
                    `);
          if (j == res.innings[i].batting.length - 1) break;
        }
          ...
          $(`#t${i + 1} table:last-child`).append(`
                        ...
                        <td>${
                          ...
                          res.innings[i].bowling[j].name
                           ...
                           <td>${res.innings[i].bowling[j].wickets}-${
            res.innings[i].bowling[j].runs
          }</td>
                           <td>${res.innings[i].bowling[j].overs.toFixed(
                             1
                           )}</td>
                        </tr>
                    `);
          if (j == res.innings[i].bowling.length - 1) break;
        }
      }
      ...
      for (let i = 0; i < 2; i++) {
        ...
        $(`#t${i + 1}-block div`).css(
          "background-color",
          res.match[`team${i + 1}`] == res.team[0].name
            ? res.team[0].color
            : res.team[1].color
        );
        $(`.c${i + 1} tr th`).css("background-color", res.team[i].color);
        $(`.c${i + 1} tr:last-child td`).css(
          "border-bottom",
          `3px solid ${res.team[i].color}`
        );
        $(`.c${i + 1} tr:nth-child(odd)`).css(
          "background-color",
          res.team[i].color + "30"
        );
        $(`.summaryButtons li`)
          .eq(i + 1)
          .html(res.team[i].abbr + " INNINGS");
        $(`#tab4 span`)
          .eq(i)
          .html(res.team[i].abbr + " INNINGS");
        ...
        $(".tables")
          .eq(i)
          .find("table tr td")
          .css("background-color", res.team[i].color + "40");
        $(".tables")
          .eq(i)
          .find("table tr td:first-child")
          .css("background-color", res.team[i].color + "20");
      }

      let labels = new Array(res.match.overs);
      for (let i = 0; i < labels.length; i++) labels[i] = i + 1;
      ...
      $(".toss span")
        .eq(1)
        .html(
          res.match.toss == res.team[0].abbr
            ? res.team[0].name
            : res.team[1].name
        );
      ...
      $(".umpires span")
        .eq(1)
        .html(res.match.umpires[0] + ", " + res.match.umpires[1]);
      ...
      for (let i = 0; i < 2; i++) {
        ...
        res.team[i].players.forEach((player, j) => {
          ...
          if (res.team[i].captain == j) playing11 += " (C)";
          if (res.team[i].vice_captain == j) playing11 += " (VC)";
          if (res.team[i].keeper == j) playing11 += " (WK)";
          playing11 += ", ";
        });
        $(`.team${i + 1} span`)
          .eq(0)
          .html(`${res.team[i].name} Playing 11 - `);
        $(`.team${i + 1} span`)
          .eq(1)
          .html(playing11.slice(0, -2));
      }
    });
